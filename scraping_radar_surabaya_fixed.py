...
Tanggal: 2024
Diperbaiki: Sistem pencarian keyword yang lebih akurat
"""
...
def is_article_relevant_by_content(title, content, keyword):
    ...
    # Cek apakah keyword utama ada dalam teks
    if keyword_lower in full_text:
        return True
    
    # Cek setiap kata dalam keyword
    ...
    for kw in keywords:
        if len(kw) > 3:
            ...
            variations = [kw, f"ber{kw}", f"me{kw}", f"{kw}an", f"{kw}nya", f"ke{kw}an"]
...
def main():
    ...
    print("🔧 Diperbaiki: Sistem pencarian keyword yang lebih akurat")
